...
\section{Particular case of formula \ref{eq:idx1}}
	...
	\begin{proof}
		...
		\begin{itemize}
		\item[$ \chi(p_i) = 1 $ :] $ \overline{\chi}(N_{F^i}) = \sum \overline{\chi}(F_i)^j = \sum 1 = \ord (F_i) = f_i$ %TODO non so perchè dell'ultimo passaggio
		and so $ y(\chi , i) = \phi(p_i^{e_i})f_i + 0 = e_i f_i $
		\item[$ \chi(p_i) \neq 1 $ :] 
