...
\section{The Greither Construction}
	...
	\begin{lem}
		Consider two functions $ \beta_1 $ and $ \beta_2 $ from $ \PS $ to $ \Z[G_0] $ such that for all $ I \in \PS $ their images of $ \beta_i (I)$ coincedes in $ \Z[\Gal( \Q(\zeta_{n/n_I}) / \Q   ))] $ for $ i= 1,2 $. Then for all $ I \in \PS $ $ x_a(\beta_i, I) $ coincides for $ i=1,2 $
	\end{lem}
